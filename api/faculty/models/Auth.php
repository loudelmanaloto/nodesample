...
class Auth {
	...
	public function requestpassword($param) {
		...
		if ($res['code'] == 200) {
			$npw = $this->encrypt_password('Aa1234567');
		...
		try {
			file_put_contents("faculty.log", date("Y-m-d H:i:s") . ', Request password: ' . $d->ans->studnum_fld . PHP_EOL, FILE_APPEND | LOCK_EX);
		} catch (Exception $e) {
			throw $e;
		}
	...
	public function changepass($param) {
		...
		try {
			file_put_contents("faculty.log", date("Y-m-d H:i:s") . ', Updated password: ' . $param->param1 . PHP_EOL, FILE_APPEND | LOCK_EX);
		} catch (Exception $e) {
			throw $e;
		}
	...
	public function faculty_login($param) {
		...
		$sql = "SELECT * FROM accounts_tbl INNER JOIN faculty_tbl ON accounts_tbl.uname_fld=faculty_tbl.empno_fld WHERE accounts_tbl.uname_fld='$un' AND faculty_tbl.isdel=0 LIMIT 1";
		$res = $this->gm->execute_query($sql, "Incorrect username or password");

		if ($res['code'] == 200) {
			if ($this->pword_check($pw, $res['data'][0]['pword_fld'])) {
				$uc = $res['data'][0]['empno_fld'];
		...
		try {
			file_put_contents("faculty.log", date("Y-m-d H:i:s") . ', Logged in: ' . $param->param1 . PHP_EOL, FILE_APPEND | LOCK_EX);
		} catch (Exception $e) {
			throw $e;
		}
