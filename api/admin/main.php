...
case "POST":
	...
	echo response($auth->instructor_login($d));

	break;



	case "getfacultyclass":

		if ($auth->authorized()) {

			echo response($get->get_classlist(bd($req[1]), bd($req[2]), bd($req[3])));
	...
	case "getclassmembers":
	    ...
	    if ($auth->authorized()) {
			...
			echo response($get->get_studentmember(bd($req[1])));
    ...
    case "getstudentclass":
    	...
    	if ($auth->authorized()) {
			...
			echo response($get->get_studentclass(bd($req[1]), bd($req[2]), bd($req[3])));
    ...
    case "studentinfo":
    	...
    	if ($auth->authorized()) {
			...
			echo response($get->get_studentdetails(bd($req[1])));
    ...
    case "updatefacultyinfo":

    	$d = jd(base64_decode(file_get_contents("php://input")));

    	if ($auth->authorized()) {

			echo response($post->update_faculty(bd($req[1]), $d));
    ...
    case "facultychangepass":

    	$d = jd(base64_decode(file_get_contents("php://input")));

    	if ($auth->authorized()) {

			echo response($auth->change_facultypass($d));
	...
	case "getannouncements":
		...
		if ($auth->authorized()) {
			...
			echo response($get->get_announcements($req[1], base64_decode($req[2])));
