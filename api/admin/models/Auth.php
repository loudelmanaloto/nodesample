	...
	class Auth {
		...
		public function student_login($param) {
			...
			$sql = "SELECT students_tbl.studnum_fld, students_tbl.fname_fld, students_tbl.lname_fld, students_tbl.emailadd_fld, students_tbl.dept_fld, students_tbl.profilepic_fld, students_tbl.program_fld, accounts_tbl.pword_fld FROM students_tbl INNER JOIN accounts_tbl USING(studnum_fld) WHERE studnum_fld = '$un' AND accounts_tbl.isdeleted_fld=0 LIMIT 1";

			$res = $this->gm->execute_query($sql, "Incorrect username");
			...
			if($res['code'] == 200) {
				...
				if($this->pword_check($pw, $res['data'][0]['pword_fld'])) {
					...
					$dp = $res['data'][0]['dept_fld'];

					$cs = $res['data'][0]['program_fld'];

					$img = $res['data'][0]['profilepic_fld'];
					...
					try {
						...
						file_put_contents("students.log", date("Y-m-d H:i:s").', Logged in GC LAMP: '.$res['data'][0]['studnum_fld'].PHP_EOL, FILE_APPEND | LOCK_EX);
					...
					$payload = array("id"=>$uc, "fullname"=>$fn, "key"=>$tk, "dept"=>$dp, "img"=>$img, "course"=>$cs);
					...
					try {
						...
						file_put_contents("students.log", date("Y-m-d H:i:s").', Attempted to Log in GC LAMP: '.$un.PHP_EOL, FILE_APPEND | LOCK_EX);
		...
		public function instructor_login($param){
			...
			$sql = "SELECT * FROM faculty_tbl WHERE empnum_fld='$un' AND isdeleted_fld=0 LIMIT 1";
			...
			if($res['code'] == 200) {
				...
				if($this->pword_check($pw, $res['data'][0]['pword_fld'])) {
					...
					//Modified Owen Jasper Vargas 12:55 12/09/2020
					...
					$this->gm->execute_query($sql, "failed to update");
						...
						$payload = array("id"=>$uc, "fullname"=>$fn, "dept" => $dept, "email" => $ue, "key"=>$tk, "img"=>$dir, "role"=>$ro);
			...
			try {
				...
				file_put_contents("faculty.log", date("Y-m-d H:i:s").', Logged in LMS: '.$param->param1.PHP_EOL, FILE_APPEND | LOCK_EX);
		...
		//Modified September 8 2020: Owen



		public function change_facultypass($param){
			...
			$sql = "SELECT * FROM faculty_tbl WHERE empnum_fld='$empnum_fld'";
			...
			if ($res['code'] == 200) {
				...
				if ($this->pword_check($oldpass, $res['data'][0]['pword_fld'])) {
					...
					$res = $this->gm->update('faculty_tbl', array("pword_fld" => $newpass), "empnum_fld='$empnum_fld'");
					...
					if ($res['code'] == 200) {
						...
						file_put_contents("students.log", date("Y-m-d H:i:s") . ', Password changed: ' . $empnum_fld . PHP_EOL, FILE_APPEND | LOCK_EX);
				...
				file_put_contents("students.log", date("Y-m-d H:i:s") . ', Attempted to change password: ' . $empnum_fld . PHP_EOL, FILE_APPEND | LOCK_EX);
